...
func TestBlockService(t *testing.T) {
	...
	bs1, err := bitswap.NewBlockService(ctx, nil, bswap1)
	require.NoError(t, err)
	defer bs1.Close()
	...
	bs2, err := bitswap.NewBlockService(ctx, nil, bswap2)
	require.NoError(t, err)
	defer bs2.Close()
	...
	payload := []byte("Hello, BlockService!")
	c, err := bs1.AddBlockRaw(ctx, payload)
	require.NoError(t, err)

	receive, err := bs2.GetBlockRaw(ctx, c)
	require.NoError(t, err)
	require.Equal(t, payload, receive)
